/******** CONFIG ********/
const DEMO_MODE = false; // auto demo if camera not found
...
function loadReport(){
  ...
  students.forEach(s=>{
    reportTable.innerHTML +=
      `<tr><td>${s.name}</td><td>${attendance[s.id]}</td></tr>`;
  });
}
...
function loadAnalysis(){
  let p = 0;
  students.forEach(s=>{
    if(attendance[s.id].startsWith("Present")) p++;
  });
  total.innerText = students.length;
  present.innerText = p;
  absent.innerText = students.length - p;
}
